...
@media only screen 
  ...
  and (-webkit-min-device-pixel-ratio: 2) {
	...
	// adds server name to error msg
	$(document).ready(function() {
		var params = new URLSearchParams(document.location.search);
		var sv = params.get("did");

		var sv_name = "Server"
		switch(sv) {
		  case "8177":
			sv_name = "RPG Survival server"
			break;
		  case "8174":
			sv_name = "Skyblock server"
			break;
		  case "8170":
			sv_name = "Anarchy server"
			break;
		  case "8167":
			sv_name = "Hunger Games server"
			break;
		  default:
			break;
		} 
		
		$("div.content").html("<h1>"+sv_name+" is under maintenance.</h1>");
	});
	</script>
